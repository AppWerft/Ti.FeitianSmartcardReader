...
public class SubCharSeq extends AbstractSequence<Char> implements CharSeq, RandomAccess, java.io.Externalizable
{
  CharSequence base;
  int start;
  int end;
  ...
  private SubCharSeq subCharSeq(int start, int end) {
    ...
    if ((start < 0) || (end < start) || (end > sz))
      throw new IndexOutOfBoundsException();
    return new SubCharSeq(base, this.start + start, this.start + end);
  }
