  ...
  extends Attribute
{
  ...
  private int fixLine(int sourceLine, int index)
  {
    ...
    if ((index == 5 * (lineCount - 1)) || ((index == 0) && (sourceLine < lines[8])))
    {

      lines[(index + 2)] = (sourceLine - sourceMin + 1);
      return sourceLine + delta;
    }
    return -1;
  }
  ...
  int fixLine(int sourceLine)
  {
    if (curLineIndex >= 0)
    {
      int outLine = fixLine(sourceLine, curLineIndex);
      if (outLine >= 0)
        return outLine;
    }
    int i5 = 0;
    ...
    for (int i = 0; i < lineCount; i++)
    {
      ...
      {
        int outLine = fixLine(sourceLine, i5);
        if (outLine >= 0)
        {
          curLineIndex = i5;
          return outLine;
    ...
    int outputStartLine; int outputStartLine; if (i5 == 0) {
  ...
  void addFile(String fname)
  {
    ...
    String fentry; String fentry; if (slash >= 0)
    {
      String fpath = fname;
