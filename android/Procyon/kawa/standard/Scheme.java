...
public class Scheme extends LispLanguage
{
    ...
    public static final Scheme r5rsInstance;
    public static final Scheme r6rsInstance;
    public static final Scheme r7rsInstance;
    ...
    private static Scheme newStandardInstance(final int standardToFollow) {
        final Scheme instance = new Scheme(Scheme.kawaEnvironment);
        instance.standardToFollow = standardToFollow;
        return instance;
    }
    
    public static synchronized Scheme getR5rsInstance() {
        return Scheme.r5rsInstance;
    }
    
    public static synchronized Scheme getR6rsInstance() {
        return Scheme.r6rsInstance;
    }
    
    public static synchronized Scheme getR7rsInstance() {
        return Scheme.r7rsInstance;
    }
    
    public static Environment builtin() {
        return Scheme.kawaEnvironment;
    }
    
    private void initScheme() {
        this.environ = Scheme.nullEnvironment;
        ...
        Scheme.nullEnvironment.setLocked();
        this.environ = Scheme.r4Environment;
        ...
        Scheme.r4Environment.setLocked();
        this.environ = Scheme.r5Environment;
        ...
        Scheme.r5Environment.setLocked();
        this.environ = Scheme.r6Environment;
        ...
        Scheme.r6Environment.setLocked();
        this.environ = Scheme.kawaEnvironment;
        ...
        int i = Scheme.uniformVectorTags.length;
        while (--i >= 0) {
            final String tag = Scheme.uniformVectorTags[i];
        ...
        Scheme.kawaEnvironment.setLocked();
        final int withServlets = HttpRequestContext.importServletDefinitions;
        if (withServlets > 0) {
            try {
    ...
    protected Scheme(final Environment env) {
        this.environ = env;
    }
    ...
    public static Object eval(final String string, final Environment env) {
        return eval(new CharArrayInPort(string), env);
    }
    
    public static Object eval(final InPort port, final Environment env) {
        ...
        try {
            ...
            final Object body = ReaderParens.readList(lexer, null, 0, 1, -1, -1);
    ...
    public static Object eval(final Object sexpr, final Environment env) {
        try {
    ...
    public LispReader getLexer(final InPort inp, final SourceMessages messages) {
        final LispReader reader = super.getLexer(inp, messages);
        if (reader.getReadCase() == '\0' && this.standardToFollow == 5) {
    ...
    public static Type getTypeValue(final Expression exp) {
    ...
    protected synchronized HashMap<String, Type> getTypeMap() {
        if (this.types == null) {
            (this.types = new HashMap<String, Type>(128)).put("boolean", Scheme.booleanType);
            this.types.put("parameter", Compilation.typeLocationProc);
            this.types.putAll(super.getTypeMap());
            int i = Scheme.uniformVectorTags.length;
            while (--i >= 0) {
                final String tag = Scheme.uniformVectorTags[i];
                final String cname = "gnu.lists." + tag.toUpperCase() + "Vector";
    ...
    public String formatType(final Type type) {
        if (type instanceof LazyType) {
            final LazyType ltype = (LazyType)type;
            return this.formatType(ltype.getRawType()) + '[' + this.formatType(ltype.getValueType()) + ']';
        }
        if (type instanceof MultValuesType) {
            final MultValuesType mtype = (MultValuesType)type;
            final StringBuilder sbuf = new StringBuilder();
            sbuf.append("values[");
            for (int n = mtype.getValueCount(), i = 0; i < n; ++i) {
                if (i > 0) {
                    sbuf.append(' ');
                }
                final Type etype = mtype.getValueType(i);
                sbuf.append((etype == null) ? "unspecified" : this.formatType(etype));
            }
            sbuf.append(']');
            return sbuf.toString();
        }
        if (type instanceof GenArrayType) {
            final GenArrayType atype = (GenArrayType)type;
            final StringBuilder sbuf = new StringBuilder("array");
            final int rank = atype.rank();
            if (rank >= 0) {
                sbuf.append(rank);
            }
        ...
        if (this.typeToStringMap == null) {
            this.typeToStringMap = new HashMap<Type, String>();
            for (final java.util.Map.Entry<String, Type> e : this.getTypeMap().entrySet()) {
                this.typeToStringMap.put(e.getValue(), e.getKey());
            }
        }
        final String str = this.typeToStringMap.get(type);
        if (str != null) {
            return str;
        }
    ...
    public static Type exp2Type(final Expression exp) {
        return getInstance().getTypeFor(exp);
    }
    
    public Symbol asSymbol(final String ident) {
    ...
    public ReadTable createReadTable() {
        ...
        final int std = this.standardToFollow;
        final ReaderDispatch dispatchTable = (ReaderDispatch)tab.lookup(35);
        final ReaderDispatchSyntaxQuote sentry = new ReaderDispatchSyntaxQuote();
    ...
    static {
        ...
        r4Environment = Environment.make("r4rs-environment", Scheme.nullEnvironment);
        r5Environment = Environment.make("r5rs-environment", Scheme.r4Environment);
        r6Environment = Environment.make("r6rs-environment", Scheme.r5Environment);
        kawaEnvironment = Environment.make("kawa-environment", Scheme.r6Environment);
        instance = new Scheme(Scheme.kawaEnvironment);
        r5rsInstance = newStandardInstance(5);
        r6rsInstance = newStandardInstance(6);
        r7rsInstance = newStandardInstance(7);
        booleanType = new LangPrimType(Type.booleanType, Scheme.instance);
        applyToArgs = new ApplyToArgs("apply-to-args", Scheme.instance);
        apply = new Apply("apply", Scheme.applyToArgs);
        instanceOf = new InstanceOf(Scheme.instance, "instance?");
        not = new Not(Scheme.instance, "not");
        isEq = new IsEq(Scheme.instance, "eq?");
        isEqv = new IsEqv(Scheme.instance, "eqv?", Scheme.isEq);
        isEqual = new IsEqual(Scheme.instance, "equal?");
        map = new Map(true, Scheme.applyToArgs, Scheme.isEq);
        forEach = new Map(false, Scheme.applyToArgs, Scheme.isEq);
        numEqu = NumberCompare.make(Scheme.instance, "=", 8);
        numGrt = NumberCompare.make(Scheme.instance, ">", 16);
        numGEq = NumberCompare.make(Scheme.instance, ">=", 24);
        numLss = NumberCompare.make(Scheme.instance, "<", 4);
        numLEq = NumberCompare.make(Scheme.instance, "<=", 12);
        isOdd = new NumberPredicate(Scheme.instance, "odd?", 1);
        isEven = new NumberPredicate(Scheme.instance, "even?", 2);
        uniformVectorTags = new String[] { "s8", "s16", "s32", "s64", "u8", "u16", "u32", "u64", "f32", "f64" };
        emptyStringLeft = new String();
        emptyStringRight = new String();
        Scheme.instance.initScheme();
    }
}
