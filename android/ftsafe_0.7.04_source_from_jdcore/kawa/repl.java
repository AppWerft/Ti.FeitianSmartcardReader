...
public class repl extends gnu.mapping.Procedure0or1
{
  ...
  protected void error(String message) { if (messagePrefix != null)
  ...
  static void checkInitFile()
  {
    if (homeDirectory == null) {
      ...
      if ((initFile != null) && (initFile.exists()) && 
        (!Shell.runFileOrClass(initFile.getPath(), true, 0)))
        System.exit(-1);
    }
  }
  ...
  static File getInitFile() {
    ...
    if (homeDirectory != null) {
      Object scmHomeDirectory = new gnu.lists.FString(homeDirectory);
  ...
  static boolean shutdownRegistered = gnu.kawa.io.WriterManager.instance.registerShutdownHook();
  ...
  public int processArgs(String[] args, int iArg, int maxArg, boolean argsOnly) {
    ...
    while (iArg < maxArg) {
      ...
      if ((arg.equals("-c")) || (arg.equals("-e"))) {
        ...
        Throwable ex = Shell.run(language, Environment.getCurrent(), new gnu.kawa.io.CharArrayInPort(expr), OutPort.outDefault(), OutPort.errDefault(), messages);
        ...
        if (arg.equals("\\"))
        {
          ...
          SourceMessages messages = new SourceMessages();
          try {
            InputStream fstream = new java.io.BufferedInputStream(new java.io.FileInputStream(filename));
            int ch = fstream.read();
            if (ch == 35) {
              StringBuffer sbuf = new StringBuffer(100);
              ArrayList<String> xargs = new ArrayList(10);
              int state = 0;
          ...
          if ((arg.equals("--output-format")) || (arg.equals("--format")))
          {
            ...
            int port; int port; if (portArg.equals("-")) {
              port = 0;
            } else {
              try {
            ...
            try {
              Socket socket = new Socket(java.net.InetAddress.getByName(null), port);
            ...
            } else if ((arg.equals("--debug-print-anf")) && (Compilation.enableANF))
            {
              ...
              kawa.lang.SyntaxPattern.printSyntaxPatternMatch = true;
            ...
            {
              if ((argsOnly) || ((something_done) && (!usedActionArgCount))) {
                break;
              }
              String filename = arg;
              getLanguageFromFilenameExtension(filename);
  ...
  public static void compileFiles(String[] args, int iArg, int maxArg)
  {
    ...
    for (int i = iArg; i < maxArg; i++) {
      ...
      try {
        ...
        if (compilationTopname != null) {
          String cname = gnu.expr.Mangling.mangleNameIfNeeded(compilationTopname);
    ...
    for (int i = iArg; i < maxArg; i++) {
      ...
      try {
        System.err.println("(compiling " + arg + " to " + mainClass.getName() + ')');
  ...
  public static void checkDomTerm()
  {
    ...
    if (dversion != null) {
      if (dversion.indexOf("err-handled;") < 0)
        gnu.kawa.io.TermErrorStream.setSystemErr(false);
  ...
  private static Throwable startJfxConsole() {
    try {
      ...
      Class.forName("org.domterm.javafx.WebTerminalApp").getMethod("startApp", new Class[] { Class.forName("org.domterm.Backend"), String.class, [Ljava.lang.String.class }).invoke(null, new Object[] { replBackend, "Kawa (DomTerm)", null });
  ...
  public static String startGuiConsole(String command) {
    try {
      ...
      Object result = Class.forName("kawa.DomTermBackend").getMethod("startDomTermConsole", new Class[] { String.class }).invoke(null, new Object[] { command });
  ...
  public static String browseManual(String path, String browserCommand) {
    try { String kawaHome = System.getProperty("kawa.home");
      if (kawaHome == null)
      ...
      gnu.kawa.servlet.KawaHttpHandler.addStaticFileHandler("/kawa-manual/", pathPrefix, defaultUrl, true);
      
      int htport = 0;
      com.sun.net.httpserver.HttpServer httpHandler = gnu.kawa.servlet.KawaHttpHandler.startServer(htport, System.err);
      ...
      if (browserCommand.equals("browser")) {
        ...
        Desktop.getDesktop().browse(new java.net.URI(webUrl));
        return null;
      }
