...
public abstract class RatNum extends RealNum
{
  ...
  {
    return numerator().isZero() ? -1 : !denominator().isZero() ? 1 : 0;
  }
  ...
  public final RatNum rneg() { return neg(this); }
  ...
  public static int compare(RatNum x, RatNum y) {
    ...
    IntNum right;
    IntNum left; IntNum right; if ((xd.isZero()) && (yd.isZero())) {
      IntNum left = xn;
      right = yn;
    } else {
  ...
  public Numeric power(IntNum y)
  {
    ...
    if (words == null)
    {
      IntNum num = IntNum.power(numerator(), ival);
      IntNum den = IntNum.power(denominator(), ival);
